...
class Ticket {
  ...
  constructor(original) {
    ...
    this.responder = Ticket.contacts.find(item => item.id === original.responder_id);
    this.responderDisplay = _.get(this.responder, 'email', '-');
    this.requester = Ticket.contacts.find(item => item.id === original.requester_id);
...
window.setTimeout(()=>{Ticket.now = dayjs();}), 1000 * 60);
